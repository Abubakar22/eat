...
#Defining variables
appdir  = '/app/eatcicd'
containerdir = "#{appdir}/cdcontainer"
jenkinsdir = "#{appdir}/.cdjenkins"
user_for_app = 'cdjenkins'
grp_for_app = 'cdjenkins'
jenkins_source = node.default['eatcicd']['jenkins_source']
jenkins_checksum = node.default['eatcicd']['jenkins_checksum']
service_name = 'cdjenkins'
...
tomcat_service service_name do
  ...
  action [:start, :enable]
  env_vars [{'CATALINA_BASE' => containerdir}, {'CATALINA_PID' => "#{containerdir}/#{service_name}.pid"}, {'JAVA_OPTS' => '-Djenkins.install.runSetupWizard=false -Dport.shutdown=9005 -Dport.http=9090'}, {'JENKINS_HOME' => "#{jenkinsdir}"}]
  sensitive true
  tomcat_user user_for_app
  tomcat_group grp_for_app
end
...
cookbook_file "#{jenkinsdir}/jenkins.CLI.xml" do
  ...
  group grp_for_app
  action :create
  notifies :restart, 'tomcat_service[cdjenkins]'
end
